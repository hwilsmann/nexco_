...
<template>
  <section class="bg-white lg:pt-24">
    <div class="mx-auto max-w-content px-5 sm:px-6 lg:px-0">
      <span class="inline-block rounded-full bg-brand-mint/50 px-4 py-1.5 text-xs font-medium text-forest-950/70">Depoimentos</span>

      <div class="mt-8 grid gap-10 sm:mt-10 lg:grid-cols-[minmax(0,1fr)_480px] lg:gap-12">
        <div class="min-w-0">
          <h2 class="text-3xl font-bold leading-[1.2] text-forest-950 sm:text-4xl lg:text-[2.5rem]">O sucesso dos nossos clientes é a nossa melhor referência</h2>

          <div class="mt-8 grid grid-cols-1 gap-4 sm:mt-10 sm:gap-6 md:grid-cols-2 items-start">
            <div v-for="testimonial in visibleTestimonials" :key="testimonial.author" class="min-w-0 rounded-2xl bg-brand-mint/35 p-4 sm:p-5">
          ...
          <div class="mt-6 flex justify-end gap-3 sm:mt-8">
            <button type="button" aria-label="Depoimento anterior" class="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-forest-900 text-white transition-opacity hover:opacity-80" @click="prev">
            ...
            <button type="button" aria-label="Próximo depoimento" class="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-forest-900 text-white transition-opacity hover:opacity-80" @click="next">
        ...
        <div class="relative h-[420px] w-full overflow-hidden rounded-[24px] sm:h-[480px] sm:rounded-[28px] lg:h-[543px]">
          ...
          <div class="absolute bottom-4 left-4 right-4 flex items-start justify-between gap-4 rounded-2xl bg-forest-950 p-4 text-white sm:bottom-6 sm:left-6 sm:right-6 sm:p-5 lg:bottom-8 lg:left-8 lg:right-8">
            <div class="min-w-0">
              <p class="text-4xl font-semibold leading-none text-brand-green sm:text-[2.75rem] lg:text-[3.125rem]">+100</p>
              <p class="mt-1 max-w-[220px] text-xs leading-relaxed text-white sm:max-w-[260px]">empresas já contaram profissionais com a Nexco</p>
            </div>
